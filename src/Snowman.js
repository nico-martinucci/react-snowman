...
function Snowman({
  ...
  let letterButtonsOrYouLose;

  if (nWrong >= 6) {
    letterButtonsOrYouLose = <p className="Snowman-loss">You lose!</p>
  } else {
    letterButtonsOrYouLose = <p className="Snowman-buttons">{generateButtons()}</p>;
  }
  ...
  return (
      <div className="Snowman">
        ...
        <p className="Snowman-guesses">{ nWrong }</p>
        <p className="Snowman-word">{guessedWord()}</p>
        { letterButtonsOrYouLose }
      </div>
  );
}
